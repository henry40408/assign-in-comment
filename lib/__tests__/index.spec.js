...
const { assignInComment, assignInPullRequest, extractAssignees } = require('../index')

let robot = { log: jest.fn() }

function mockCommentContext () {
  return {
    payload: {
      action: 'action',
      comment: { id: 1048576 }
    ...
    github: {
      issues: {
        ...
        getComment: jest.fn().mockReturnValue({
          data: {
            body: '/handover @foo @baz',
            user: {
              login: 'bar'
            }
          }
...
function mockPullRequestContext () {
  return {
    config: async () => {},
    payload: {
      action: 'action'
...
describe('assignInComment', () => {
  test('unassign comment author and assign users', async () => {
...
describe('assignInPullRequest', () => {
  test('assign who are not assigned to be assignees', async () => {
    const context = mockPullRequestContext()
    await assignInPullRequest(robot, context)
